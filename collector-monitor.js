...
async function load(){try{const r=await fetch('/api/status?'+Date.now());const d=await r.json();const s=d.status||{};const age=Date.now()-Date.parse(s.updatedAt||0);const alive=Number.isFinite(age)&&age<15*60*1000;$('live').textContent=alive?'● 수집기 정상':'● 상태 오래됨';$('live').className='live'+(alive?'':' off');$('phase').textContent='자동 새로고침 3초 · '+dt(d.now);$('task').textContent=taskText(s.phase);$('sub').textContent=s.message||'';$('checked').textContent=fmt(s.lastRun?.checked);$('success').textContent=fmt(s.lastRun?.success);$('docs').textContent=fmt(s.lastRun?.docsAdded);$('patches').textContent=fmt(s.lastPatchCount);$('candidates').textContent=fmt(s.candidates);$('malformed').textContent=fmt(s.skippedMalformed);$('pending').textContent=fmt(s.pendingSkipped);$('total').textContent=fmt(d.canonical?.itemCount);$('stage').textContent=s.phase||'-';$('started').textContent=dt(s.startedAt);$('finished').textContent=dt(s.finishedAt);$('error').textContent=s.lastRun?.error||s.error||'없음';$('delta').textContent=fmt(d.delta?.files)+'개 파일 / '+fmt(d.delta?.patches)+'건 패치';}catch(e){$('live').textContent='● 모니터 연결 실패';$('live').className='live off';}}load();setInterval(load,3000);
</script></body></html>`;
