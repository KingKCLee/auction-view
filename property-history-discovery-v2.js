...
function usable(r){return Boolean(text(r.address).trim()&&text(r.courtCode).trim()&&text(r.caseNumber).trim()&&(text(r.usage).trim()||Number(r.appraisedPrice)>0||Number(r.minimumPrice)>0||text(r.saleDate).trim()))}
function norm(i){
 ...
 const row={id:`${courtCode}|${caseNumber}|${itemNumber}`,courtCode,courtName,caseNumber,internalCaseNumber,itemNumber,
  usage:text(i.usage||raw.usgNm||raw.dspslUsgNm),usageCodes:i.usageCodes||null,address:text(i.address||raw.st||raw.userSt||raw.userStPrint),regionSido:'',regionSigungu:'',regionCodes:i.regionCodes||null,buildingName:text(i.buildingName||raw.buldNm),propertyDescription:text(i.propertyDescription||i.remarks),
  ...
  coverage:{base_info:0,schedule:0,winning_price:0,photos:0,status_report:0,sale_statement:0,appraisal_summary:0,appraisal_pdf:0,transactions:0,building_registry:0,land_use:0,rights:0},events:[],components:[],documents:[],firstSeenAt:new Date().toISOString(),lastSeenAt:new Date().toISOString(),source:'대한민국 법원경매정보 물건검색'};
 row.coverage.base_info=usable(row)?1:0;return row;
}
