...
async function openNoticeMonth(monthKey){
 const attempts=[];
 const http=new lib.CourtAuctionHttpClient({timeoutMs:30000,minDelayMs:3000,jitterMs:1200,maxCallsPerSession:7});
 try{const res=await lib.searchSaleNotices({date:monthKey,courtCode:'',bidType:'date',client:http,includeRaw:true});return{res,client:http,transport:'http'}}catch(e){attempts.push(`http:${e.code||''}:${e.message||e}`);if(e?.code==='BLOCKED')return{error:e,attempts,transport:'blocked'}}
 if(typeof lib.CourtAuctionPlaywrightClient==='function'){
   ...
   try{const res=await lib.searchSaleNotices({date:monthKey,courtCode:'',bidType:'date',client:browser,includeRaw:true});return{res,client:browser,transport:'browser'}}catch(e){attempts.push(`browser:${e.code||''}:${e.message||e}`);try{await browser.close()}catch{};return{error:e,attempts,transport:'failed'}}
 }
...
async function main(){
 ...
 const monthKey=ym(y,m);let searched=false,processed=0,successful=0,err=null,list=[],transport='none',client=null,attempts=[];
 const opened=await openNoticeMonth(monthKey);transport=opened.transport;attempts=opened.attempts||[];client=opened.client||null;
 if(opened.res){list=opened.res?.items||[];searched=true;state.saleNoticeFailures[monthKey]=0}else{const e=opened.error;err=`search ${monthKey}: ${e?.message||e}`;state.saleNoticeFailures[monthKey]=Number(state.saleNoticeFailures[monthKey]||0)+1;}
 if(searched){
   if(!list.length){const [py,pm]=prev(y,m);y=py;m=pm;offset=0;}
     ...
     for(let i=offset;i<list.length&&processed<BATCH;i++){
       ...
       try{const d=await lib.getSaleNoticeDetail(n,{client,includeRaw:true});for(const item of d?.items||[]){const z=normalize(item,n);if(z)map.set(z.id,map.has(z.id)?merge(map.get(z.id),z):z)}successful++;state.saleNoticeFailures[nk]=0;offset=i+1;await sleep(3200+Math.floor(Math.random()*1400));}
 ...
 const added=Math.max(0,map.size-before);stats.generatedAt=new Date().toISOString();stats.itemCount=out.length;stats.coverage={...(stats.coverage||{}),base_info:out.length};stats.saleNoticeBackfill={cursor:{year:y,month:m,offset},processedNotices:processed,successfulNotices:successful,newUniqueItems:added,deferredCount:state.saleNoticeDeferred.length,lastError:err,transport,attempts,source:'대한민국 법원경매정보 매각공고',finishedAt:new Date().toISOString()};write(STATS,stats);
