...
## 부록 C — 검색 필터 (2026-09-15 실측)

원본 `/api/auction/list` 가 거를 수 있는 축. 카드 인덱스에 이미 실려 있던 값이라
인덱스 크기를 늘리지 않고 필터만 열었다(409KB / 500KB 상한).

| 질의값 | 대상 | 실측 건수(전체 12,111 기준) |
|---|---|---|
| `court=` (쉼표 다중) | `courtName` | 서울중앙 438 · 서울동부 440 · 둘 다 878(합과 일치) |
| `minFailed` / `maxFailed` | `failedCount` | ≥1 6,394 · ≥3 2,811 · =0 5,717 (6,394+5,717=12,111) |
| `minAppraised` / `maxAppraised` | `appraisedPrice` | ≥10억 1,420 · ≤1억 3,139 · 3~5억 1,692 |
| `usage=` (쉼표 다중) | `usage` | 아파트 1,591 · 아파트+오피스텔 2,437 |

★법원 선택지는 **두 곳**에서 온다. 목록 응답에 실려 오는 `facets` 는 카드 인덱스
블롭에 같이 구워진 것이라 나중에 늘어난 축이 없고, `/api/auction/facets` 가 그것을
메운다(없으면 인덱스에서 뽑아 준다). 화면은 **둘을 합쳐야 한다** — 목록 쪽만 쓰면
법원 목록이 영영 빈다(집톡에서 실제로 그랬다).

★용도 표기는 같은 것을 여러 이름으로 적어 25종이다("다세대"·"다세대주택"·"빌라").
화면의 빠른 탭은 묶어서 쉼표 다중으로 보낸다 — 빌라·다세대 1,824 · 토지 3,089.

## 부록 D — 배포 사고 (2026-09-15)

`auction-view` Cloudflare Pages 의 **Git 연결이 끊겨 있어** 6일간 자동 배포가
한 번도 뜨지 않았다. 창고에는 올라갔는데 라이브는 옛 코드였다 — 커밋이 곧 배포가
아니라는 것을 화면 쪽에서 알 방법이 없어서, 새 필터를 단 화면이 "칸은 있는데 안
걸러지는" 상태로 라이브에 나갈 뻔했다.

그래서 **원본은 모르는 질의값에 400 을 낸다**(`minFailed=xyz` 등). 화면은 그 검사
하나로 원본이 새 판인지 판별해, 옛 판이면 새 칸을 잠그고 이유를 적는다. 배포가
붙는 순간 저절로 열린다. 이 규약은 앞으로 축을 더할 때도 유지한다.

수동 우회: `npx wrangler pages deploy public --project-name auction-view`
